...
- **Monorepo 拆分** - `packages/core` 拆分为 8 个独立包：
  - `@microbot/types` - 核心类型定义（MCP 兼容）
  - `@microbot/runtime` - 运行时引擎（Container、EventBus、HookSystem、Gateway）
  - `@microbot/config` - 三级配置系统
  - `@microbot/storage` - 会话存储
  - `@microbot/providers` - LLM Provider 抽象
  - `@microbot/extension-system` - 扩展发现、加载、热重载
  - `@microbot/sdk` - 聚合 SDK
  - `@microbot/server` - 服务层
